...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.packagesCommission.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.packages-commissions.update", [$packagesCommission->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="tuition_package_efk">{{ trans('cruds.packagesCommission.fields.tuition_package_efk') }}</label>
                <input class="form-control {{ $errors->has('tuition_package_efk') ? 'is-invalid' : '' }}" type="number" name="tuition_package_efk" id="tuition_package_efk" value="{{ old('tuition_package_efk', $packagesCommission->tuition_package_efk) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('commission') ? 'is-invalid' : '' }}" name="commission_id" id="commission_id">
                    @foreach($commissions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('commission_id') ? old('commission_id') : $packagesCommission->commission->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
