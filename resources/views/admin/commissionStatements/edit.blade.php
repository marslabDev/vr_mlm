...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.commissionStatement.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.commission-statements.update", [$commissionStatement->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('agent') ? 'is-invalid' : '' }}" name="agent_id" id="agent_id">
                    @foreach($agents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('agent_id') ? old('agent_id') : $commissionStatement->agent->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('agent'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total') ? 'is-invalid' : '' }}" type="number" name="total" id="total" value="{{ old('total', $commissionStatement->total) }}" step="0.01" required>
                @if($errors->has('total'))
                    <div class="invalid-feedback">
                        {{ $errors->first('total') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('commission_groups') ? 'is-invalid' : '' }}" name="commission_groups[]" id="commission_groups" multiple>
                    @foreach($commission_groups as $id => $commission_group)
                        <option value="{{ $id }}" {{ (in_array($id, old('commission_groups', [])) || $commissionStatement->commission_groups->contains($id)) ? 'selected' : '' }}>{{ $commission_group }}</option>
                    @endforeach
                </select>
