...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.mlmLevel.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.mlm-levels.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('current_plan') ? 'is-invalid' : '' }}" name="current_plan_id" id="current_plan_id" required>
                    @foreach($current_plans as $id => $entry)
                        <option value="{{ $id }}" {{ old('current_plan_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
